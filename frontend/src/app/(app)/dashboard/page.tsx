...
export default function DashboardPage() {
  ...
  const isLoading = dashboardQuery.isLoading || categoryQuery.isLoading || historyQuery.isLoading;
  const isError = dashboardQuery.isError || categoryQuery.isError || historyQuery.isError;
  const errorMessage = dashboardQuery.error || categoryQuery.error || historyQuery.error;

  return (
    <AuthGuard>
      <AppShell title="Dashboard" subtitle={`Account overview for UserID ${scopedUserId ?? "-"}`}>
        {isLoading ? <LoadingSkeleton label="Loading dashboard..." /> : null}

        {isError ? (
          <ErrorState
            detail={extractApiErrorMessage(errorMessage, "Failed to load dashboard data.")}
            onRetry={() => {
        ...
        {dashboardQuery.data && categoryQuery.data && historyQuery.data ? (
          <>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4 mb-6">
              <KpiCard label="Total Income" value={dashboardQuery.data.summary.TotalIncome} tone="success" />
              <KpiCard label="Total Expense" value={dashboardQuery.data.summary.TotalExpense} tone="warning" />
              <KpiCard
                label="Net Saving"
                value={dashboardQuery.data.summary.NetSaving}
                tone={dashboardQuery.data.summary.NetSaving >= 0 ? "success" : "danger"}
              />
              <KpiCard
                label="Active Alerts"
            ...
            <div className="mt-6 rounded-xl border border-border bg-surface p-5 shadow-sm">
              <h2 className="mb-4 text-lg font-semibold tracking-tight text-text">Spending Alerts</h2>
                    ...
                    >
                      ...
                      <span
                        className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${
                          getAlertTone(alert.AlertLevel) === "danger"
                            ? "bg-danger/10 text-danger"
                            : getAlertTone(alert.AlertLevel) === "warning"
